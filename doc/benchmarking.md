...
Quarlcoin has an internal benchmarking framework, with benchmarks
...
For benchmarking, you only need to compile `bench_quarlcoin`.  The bench runner
warns if you configure with `-DCMAKE_BUILD_TYPE=Debug`, but consider if building without
it will impact the benchmark(s) you are interested in by unlatching log printers
and lock analysis.

    cmake -B build -DBUILD_BENCH=ON
    cmake --build build -t bench_quarlcoin

After compiling quarlcoin-core, the benchmarks can be run with:

    build/bin/bench_quarlcoin
...
To monitor Quarlcoin performance more in depth (like reindex or IBD): https://github.com/quarlcoin-dev-tools/benchcoin
