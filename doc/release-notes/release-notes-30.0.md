...
Quarlcoin version v30.0 is now available from:

  <https://quarlcoincore.org/bin/quarlcoin-core-30.0/>
...
New `quarlcoin` command
---------------------

- A new `quarlcoin` command line tool has been added to make features more discoverable
  and convenient to use. The `quarlcoin` tool just calls other executables and does not
  implement any functionality on its own. Specifically `quarlcoin node` is a synonym for
  `quarld`, `quarlcoin gui` is a synonym for `quarl-qt`, and `quarlcoin rpc` is a synonym
  for `quarl-cli -named`. Other commands and options can be listed with `quarlcoin help`.
  The new `quarlcoin` command is an alternative to calling other commands directly, but it
...
- The new `quarlcoin` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `quarlcoin -m node -ipcbind=unix` it will
  ...
  (`quarl-node` instead of `quarld`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/quarlcoin/quarlcoin/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/quarlcoin/quarlcoin/blob/master/doc/multiprocess.md)),
...
- The `test_quarlcoin` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `quarlcoin` command
  as `quarlcoin test`. The `libexec/` directory also contains new `quarl-node` and
  `quarlcoin-gui` binaries which support IPC features and are called through the
  `quarlcoin` tool. In source builds only, `test_quarl-qt`, `bench_quarlcoin`, and
  `quarlcoin-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `quarlcoin` command. See `quarlcoin help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of Quarlcoin may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in Quarlcoin 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the quarl-wallet tool,
