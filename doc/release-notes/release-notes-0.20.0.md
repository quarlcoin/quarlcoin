...
Quarlcoin version 0.20.0 is now available from:

  <https://quarlcoincore.org/bin/quarlcoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `QUARLCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Quarlcoin

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Quarlcoin version 0.18.0.
...
- Testing or debugging of implementations of the Quarlcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Quarlcoin. Quarlcoin logs debug messages
...
- OpenSSL is no longer used by Quarlcoin.  (#17265)

- BIP70 support has been fully removed from Quarlcoin. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/quarlcoin/quarlcoin/issues/16599),
  [PR #16702](https://github.com/quarlcoin/quarlcoin/pull/16702), and the
  `quarld help` for more information.  This option is experimental and
...
- All custom settings configured when Quarlcoin starts are now
...
- The "Start Quarlcoin on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://quarlcoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Quarlcoin wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Quarlcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/quarlcoin-core/HWI) for
  signing.  Future versions of Quarlcoin should support a GUI option
...
- Quarlcoin's automatically-generated source code documentation is
  now available at https://doxygen.quarlcoincore.org.  (#17596)
...
- The `quarl-cli` utility used with the `-getinfo` parameter now
...
- The `quarl-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `quarlcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_quarl-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBQUARLCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_quarlcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBQUARLCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for quarl-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate quarlcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest quarlcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_quarlcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (quarlcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone quarlcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Quarlcoin files (hebasto)
...
- #17119 Fix broken quarl-cli examples (andrewtoth)
...
- #17503 Remove bitness from quarl-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.quarlcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to quarl-wallet (hel-o)
...
- #18341 Replace remaining literal QRL with `CURRENCY_UNIT` (domob1812)
...
- #16115 On quarld startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from quarl-cli -getinfo (malevolent)
