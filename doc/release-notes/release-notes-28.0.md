Quarlcoin version 28.0 is now available from:

  <https://quarlcoincore.org/bin/quarlcoin-core-28.0/>
...
Running Quarlcoin binaries on macOS requires self signing.
```
cd /path/to/quarlcoin-28.0/bin
xattr -d com.apple.quarantine quarl-cli quarl-qt quarl-tx quarl-util quarl-wallet quarld test_quarlcoin
codesign -s - quarl-cli quarl-qt quarl-tx quarl-util quarl-wallet quarld test_quarlcoin
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. Quarlcoin
...
Support for Testnet4 as specified in [BIP94](https://github.com/quarlcoin/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Quarlcoin`
to `C:\Users\Username\AppData\Local\Quarlcoin`. Quarlcoin will check the existence
...
See [JSON-RPC-interface.md](https://github.com/quarlcoin/quarlcoin/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libquarlcoinconsensus Removal
---------------------------

The libquarlcoin-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if Quarlcoin was listening for P2P connections, either using
...
- Quarlcoin will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/quarlcoin/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Quarlcoin with the configuration
...
are now required to compile Quarlcoin. (#29091, #30263)

- The minimum required glibc to run Quarlcoin is now
...
- A new `-testdatadir` option has been added to `test_quarlcoin` to allow specifying the
...
Previous releases of Quarlcoin or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
