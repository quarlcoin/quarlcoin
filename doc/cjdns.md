# CJDNS support in Quarlcoin

It is possible to run Quarlcoin over CJDNS, an encrypted IPv6 network that
...
done outside of Quarlcoin, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Quarlcoin network and individual nodes.
...
## Run Quarlcoin with CJDNS

Once you are connected to the CJDNS network, the following Quarlcoin
...
When enabled, this option tells Quarlcoin that it is running in an
...
IPv6 local network. This helps Quarlcoin perform better address management:
...
CJDNS support was added to Quarlcoin in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `quarl-cli -addrinfo` to see the
...
## CJDNS-related information in Quarlcoin

There are several ways to see your CJDNS address in Quarlcoin:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `quarl-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `quarl-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `quarl-cli help getnodeaddresses` for details.
