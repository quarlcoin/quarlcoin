...
Currently, it is possible to create a multisig wallet using Quarlcoin only.
...
Before starting this tutorial, start the quarlcoin node on the signet network.

```bash
./build/bin/quarld -signet -daemon
```

This tutorial also uses the default WPKH derivation path to get the xpubs and does not conform to [BIP 45](https://github.com/quarlcoin/bips/blob/master/bip-0045.mediawiki) or [BIP 87](https://github.com/quarlcoin/bips/blob/master/bip-0087.mediawiki).

At the time of writing, there is no way to extract a specific path from wallets in Quarlcoin. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./build/bin/quarlcoin rpc -signet createwallet "participant_${n}"
done
```

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, Quarlcoin single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in Quarlcoin will include the checksum in their output.

Note that previously at least two descriptors were usually used, one for external derivation paths and one for internal ones. Since https://github.com/quarlcoin/quarlcoin/pull/22838 this redundancy has been eliminated by a multipath descriptor with <code><0;1></code> at the [BIP-44](https://github.com/quarlcoin/bips/blob/master/bip-0044.mediawiki#change) change level expanding to external and internal descriptors when imported.
...
for ((n=1;n<=3;n++))
do
 xpubs["xpub_${n}"]=$(./build/bin/quarlcoin rpc -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))' | sed 's /0/\* /<0;1>/* ')
done
```
...
As previously mentioned, this step extracts the `m/84'/1'/0'` account instead of the path defined in [BIP 45](https://github.com/quarlcoin/bips/blob/master/bip-0045.mediawiki) or [BIP 87](https://github.com/quarlcoin/bips/blob/master/bip-0087.mediawiki), since there is no way to extract a specific path in Quarlcoin at the time of writing.
...
checksum=$(./build/bin/quarlcoin rpc -signet getdescriptorinfo $desc | jq -r '.checksum')

multisig_desc="[{\"desc\": \"${desc}#${checksum}\", \"active\": true, \"timestamp\": \"now\"}]"
```

`desc` specifies the output type (`wsh`, in this case) and the xpubs involved. It also uses BIP 67 (`sortedmulti`), so the wallet can be recreated without worrying about the order of xpubs. Conceptually, descriptors describe a list of scriptPubKey (along with information for spending from it) [[source](https://github.com/quarlcoin/quarlcoin/issues/21199#issuecomment-780772418)].
...
Documentation for these and other parameters can be found by typing `./build/bin/quarlcoin rpc -signet help importdescriptors`.
...
```bash
./build/bin/quarlcoin rpc -signet createwallet "multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/quarlcoin rpc -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/quarlcoin rpc -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/quarlcoin rpc -signet loadwallet "participant_${n}"; done
./build/bin/quarlcoin rpc -signet loadwallet "multisig_wallet_01"
```
...
```bash
receiving_address=$(./build/bin/quarlcoin rpc -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./build/bin/quarl-cli -a $receiving_address
```
...
```bash
./build/bin/quarlcoin rpc -signet -rpcwallet="multisig_wallet_01" getbalances
```

### 1.5 Create a PSBT

Unlike singlesig wallets, multisig wallets cannot create and sign transactions directly because they require the signatures of the co-signers. Instead they create a Partially Signed Quarlcoin Transaction (PSBT).

PSBT is a data format that allows wallets and other tools to exchange information about a Quarlcoin transaction and the signatures necessary to complete it. [[source](https://quarlcoinops.org/en/topics/psbt/)]

The current PSBT version (v0) is defined in [BIP 174](https://github.com/quarlcoin/bips/blob/master/bip-0174.mediawiki).

For simplicity, the destination address is taken from the `participant_1` wallet in the code above, but it can be any valid quarlcoin address.
...
```bash
balance=$(./build/bin/quarlcoin rpc -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/quarlcoin rpc -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/quarlcoin rpc -signet -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```

There is also the `createpsbt` RPC, which serves the same purpose, but it has no access to the wallet or to the UTXO set. It is functionally the same as `createrawtransaction` and just drops the raw transaction into an otherwise blank PSBT. [[source](https://quarlcointalk.org/index.php?topic=5131043.msg50573609#msg50573609)] In most cases, `walletcreatefundedpsbt` solves the problem.
...
```bash
./build/bin/quarlcoin rpc -signet decodepsbt $funded_psbt

./build/bin/quarlcoin rpc -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/quarlcoin rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/quarlcoin rpc -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/quarlcoin rpc -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/quarlcoin rpc -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/quarlcoin rpc -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/quarlcoin rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/quarlcoin rpc -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/quarlcoin rpc -signet finalizepsbt $psbt_2 | jq -r '.hex')
