# Fuzzing Quarlcoin using libFuzzer

## Quickstart guide

To quickly get started fuzzing Quarlcoin using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
## Overview of Quarlcoin fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/quarlcoin_report.pdf) includes an external overview of the status of Quarlcoin fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.
...
It is possible to specify `quarld` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`quarlcoin-core/qa-assets`](https://github.com/quarlcoin-core/qa-assets) repo.

To fuzz `process_message` using the [`quarlcoin-core/qa-assets`](https://github.com/quarlcoin-core/qa-assets) seed corpus:

```sh
$ git clone --depth=1 https://github.com/quarlcoin-core/qa-assets
...
an instrumentalized libc++, then using it to build [Quarlcoin dependencies
from source](../depends/README.md) and finally the Quarlcoin fuzz binary
...
the qa-assets repo
(https://github.com/quarlcoin-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`quarlcoin-core/qa-assets`](https://github.com/quarlcoin-core/qa-assets) repo.

Every single pull request submitted against the Quarlcoin repo is automatically tested against all inputs in the [`quarlcoin-core/qa-assets`](https://github.com/quarlcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Quarlcoin more robust.
...
# Fuzzing Quarlcoin using afl++

## Quickstart guide

To quickly get started fuzzing Quarlcoin using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Quarlcoin using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Quarlcoin using [Honggfuzz](https://github.com/google/honggfuzz):
...
Quarlcoin participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/quarlcoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://issues.oss-fuzz.com/issues?q=quarlcoin-core%20status:open).

Quarlcoin follows its [security disclosure policy](https://quarlcoincore.org/en/security-advisories/),
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_quarlcoin-core/latest).
