# User-space, Statically Defined Tracing (USDT) for Quarlcoin

Quarlcoin includes statically defined tracepoints to allow for more
...
**Note**: Quarlcoin uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Quarlcoin
...
Multiple tools can list the available tracepoints in a `quarld` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Quarlcoin, use `info probes` in `gdb`:

```
$ gdb ./build/bin/quarld
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/quarld
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/quarld
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/quarld
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Quarlcoin.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/quarld | grep NT_STAPSDT -A 4 -B 2
