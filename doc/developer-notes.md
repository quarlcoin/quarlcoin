...
Quarlcoin uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while quarld is running with the `logging`
RPC.  For instance, launching quarld with `-debug` or `-debug=1` will turn on
...
with "play quarlcoins" on a test network.
...
Quarlcoin is a multi-threaded application, and deadlocks or other
...
You can then use the `-debug=lock` configuration option at quarld startup or
`quarl-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `quarl-cli logging [] '["lock"]'`.
...
# A coverage report will now be accessible at `./build/test_quarlcoin.coverage/index.html`,
...
```shell
llvm-cov show \
    --object=build/bin/test_quarlcoin \
    --object=build/bin/quarld \
    ...
    --project-title="Quarlcoin Coverage Report"
```
...
llvm-cov show \
    ...
    --show-expansions \
    --output-dir=build/coverage_report \
    --project-title="Quarlcoin Fuzz Coverage Report"
```
...
To profile a running quarld process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep quarld` -- sleep 60
```
...
```shell
$ valgrind --suppressions=test/sanitizer_suppressions/valgrind.supp build/bin/test_quarlcoin
$ valgrind --suppressions=test/sanitizer_suppressions/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/bin/test_quarlcoin --log_level=test_suite
$ valgrind -v --leak-check=full build/bin/quarld -printtoconsole
$ ./build/test/functional/test_runner.py --valgrind
```

### Sanitizers

Quarlcoin can be compiled with various "sanitizers" enabled, which add
...
pay attention to for reviewers of Quarlcoin code.
...
- [Main thread (`quarld`)](https://doxygen.quarlcoincore.org/quarld_8cpp.html#main)
  : Started from `main()` in `quarld.cpp`. Responsible for starting up and
  shutting down the application.

- [Init load (`b-initload`)](https://doxygen.quarlcoincore.org/init_8cpp.html#initload)
...
- [CCheckQueue::Loop (`b-scriptch.xx`)](https://doxygen.quarlcoincore.org/class_c_check_queue.html#checkqueue)
  : Parallel script validation threads for transactions in blocks.

- [ThreadHTTP (`b-http`)](https://doxygen.quarlcoincore.org/httpserver_8cpp.html#http)
  : Thread to listen for RPC and REST connections.

- [HTTP worker threads (`b-http.xx`)](https://doxygen.quarlcoincore.org/httpserver_8cpp.html#http_pool)
  : Threads to service RPC and REST requests.

- [Indexer threads (`b-txidx`, `b-blkfltbscidx`, `b-coinstatsidx`, `b-txospenderidx`)](https://doxygen.quarlcoincore.org/class_base_index.html#index_sync)
  : One thread per indexer.

- [SchedulerThread (`b-scheduler`)](https://doxygen.quarlcoincore.org/class_c_scheduler.html#scheduler)
...
- [TorControlThread (`b-torcontrol`)](https://doxygen.quarlcoincore.org/class_tor_controller.html#torcontrol)
  : Thread for tor connections.

- Net threads:

  - [ThreadMessageHandler (`b-msghand`)](https://doxygen.quarlcoincore.org/class_c_connman.html#msghand)
  ...
  - [ThreadDNSAddressSeed (`b-dnsseed`)](https://doxygen.quarlcoincore.org/class_c_connman.html#dnsseed)
    : Loads addresses of peers from the DNS.

  - [ThreadMapPort (`b-mapport`)](https://doxygen.quarlcoincore.org/mapport_8cpp.html#mapport)
    : Universal plug-and-play startup/shutdown.

  - [ThreadSocketHandler (`b-net`)](https://doxygen.quarlcoincore.org/class_c_connman.html#net)
    : Sends/Receives data from peers on port 8333.

  - [ThreadOpenAddedConnections (`b-addcon`)](https://doxygen.quarlcoincore.org/class_c_connman.html#addcon)
    : Opens network connections to added nodes.

  - [ThreadOpenConnections (`b-opencon`)](https://doxygen.quarlcoincore.org/class_c_connman.html#opencon)
    : Initiates new connections to peers.

  - [ThreadI2PAcceptIncoming (`b-i2paccept`)](https://doxygen.quarlcoincore.org/class_c_connman.html#i2paccept)
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers (hh, h, l, ll, j, z, t, L) for arithmetic types.
  ...
  - *Rationale*: Quarlcoin uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Normally, these are maintained by active developers of Quarlcoin, in which case
...
a Quarlcoin subtree, so that they can be integrated quickly. Cosmetic changes
...
The tool instructions also include a list of the subtrees managed by Quarlcoin.

To fully verify or update a subtree, add it as a remote:

```sh
git remote add libmultiprocess https://github.com/quarlcoin-core/libmultiprocess.git
```
...
- src/leveldb
  - Upstream at https://github.com/google/leveldb ; maintained by Google. Open
    important PRs to the Quarlcoin subtree to avoid delay.
...
file descriptors, it will cause problems with Quarlcoin's `select()` loop, because
...
compatibility between nodes. This happened in Quarlcoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Quarlcoin's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/quarlcoin/quarlcoin/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/quarlcoin/quarlcoin/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/quarlcoin/quarlcoin/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `quarl-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `quarl-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Quarlcoin, the wallet was always
...
Quarlcoin uses a structured process for deprecating and removing features to give
...
- The RPC help text must mention the deprecation and the `-deprecatedrpc=<feature>` flag
  ...
  ```
  "\nDeprecated in v25.0, use the `newfoo` RPC instead. Start quarld with"
...
- Interface method names should be `lowerCamelCase` and standalone function names should be
  ...
  *Rationale*: consistency and friendliness to code generation tools.
  ...
  before in [#14635](https://github.com/quarlcoin/quarlcoin/pull/14635).
