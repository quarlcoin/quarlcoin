# Support for signing transactions outside of Quarlcoin

Quarlcoin can be launched with `-signer=<cmd>` where `<cmd>` is an external tool which can sign transactions and perform other functions. For example, it can be used to communicate with a hardware wallet.

Interaction with external signer uses [Partially Signed Quarlcoin Transaction (PSBT)](psbt.md).

## Example usage

The following example is based on the [HWI](https://github.com/quarlcoin-core/HWI) tool. Version 2.0 or newer is required. Although this tool is hosted under the Quarlcoin GitHub organization and maintained by Quarlcoin developers, it should be used with caution. It is considered experimental and has far less review than Quarlcoin itself. Be particularly careful when running tools such as these on a computer with private keys on it.

When using a hardware wallet, consult the manufacturer website for (alternative) software they recommend. As long as their software conforms to the standard below, it should be able to work with Quarlcoin.

Start Quarlcoin:

```sh
quarld -signer=../HWI/hwi.py
```

`quarlcoin node` can also be substituted for `quarld`.

### Device setup

Follow the hardware manufacturers instructions for the initial device setup, as well as their instructions for creating a backup. Alternatively, for some devices, you can use the `setup`, `restore` and `backup` commands provided by [HWI](https://github.com/quarlcoin-core/HWI).
...
```sh
quarlcoin rpc createwallet wallet_name="hww2" disable_private_keys=true descriptors=true external_signer=true
```

Creation of the external wallet can be confirmed with `getwalletinfo`, which will report `"external_signer": true`. These commands can also be executed using `quarl-qt` Debug Console instead of using `quarlcoin rpc` or `quarl-cli`.
...
```sh
quarl-cli -rpcwallet=<walletname> getnewaddress
quarl-cli -rpcwallet=<walletname> walletdisplayaddress <address>
```
...
Under the hood this uses a [PSBT (Partially Signed Quarlcoin Transaction)](psbt.md).

```sh
quarlcoin rpc -rpcwallet=<walletname> send outputs='{"<address>": <amount>}'
```

This constructs a PSBT and prompts your external signer to sign (will fail if it's not connected). If successful, Quarlcoin finalizes and broadcasts the transaction.
...
In order to be compatible with Quarlcoin, any signer command should conform to the specification below. This specification is subject to change. Ideally a BIP should propose a standard so that other wallets can also make use of it.

Prerequisite knowledge:
* [Output Descriptors](descriptors.md)
* Partially Signed Quarlcoin Transaction ([PSBT](psbt.md))

### Flag `--chain <name>` (required except for `enumerate`)

With `<name>` one of `main`, `test`, `signet`, `regtest`, `testnet4`.

Quarlcoin passes this flag to commands that operate on a chain-specific signer, for example `getdescriptors`, `displayaddress` and `signtx`.
...
Quarlcoin currently uses this flag for `signtx`.
...
Quarlcoin passes this flag to commands that operate on a specific external-signer wallet, for example `getdescriptors`, `displayaddress` and `signtx`.
...
## How Quarlcoin uses the Signer API
...
For external-signer wallets, spending uses `send` or `sendall`, and fee-bumping uses `bumpfee`. Quarlcoin builds a PSBT, adds key origin information, checks whether any input key origin fingerprint matches the signer, calls `<cmd> --stdin --fingerprint 00000000 --chain <name>`, and sends `signtx <psbt>` over stdin. If signatures are sufficient, it finalizes the transaction and, for broadcasting RPCs, broadcasts it. If signing cannot complete, the call fails with an error. For manual fee-bumping, use `psbtbumpfee` to obtain a PSBT for signing.
