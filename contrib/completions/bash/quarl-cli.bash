# bash programmable completion for quarl-cli(1)
# Copyright (c) 2026 The Quarlcoin developers
# See COPYING for license.

# call $quarl-cli for RPC
_quarlcoin_rpc() {
...
_quarlcoin_cli() {
    ...
    # save and use original argument to invoke quarl-cli for -help, help and RPC
    # as quarl-cli might not be in $PATH
    quarlcoin_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($quarlcoin_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_quarlcoin_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _quarlcoin_cli quarl-cli
