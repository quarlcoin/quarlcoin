...
be used in Quarlcoin with the `-asmap` option.
...
to binary before being used with Quarlcoin.
...
the `quarl-cli getnodeaddresses` command.
...
```
quarl-cli getnodeaddresses 0 > addrs.json
```
