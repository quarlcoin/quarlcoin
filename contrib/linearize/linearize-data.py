...
def calc_hash_str(blk_hdr):
    # A block hash is double SHA-256 over the serialized header
    # (CBlockHeader::GetHash via HashWriter), as in Bitcoin. This computed
    # sha3_256 instead, from a design that was replaced rather than shipped, so
    # every hash it produced named nothing.
...
class BlockDataCopier:
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            ...
            inLen = su[0] - 80 # length without header (Quarlcoin uses Bitcoin's 80-byte header)
