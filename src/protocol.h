...
#ifndef QUARLCOIN_PROTOCOL_H
#define QUARLCOIN_PROTOCOL_H
 ...
 *
 * v2 transport can be assumed rather than discovered. Quarlcoin has spoken the
 * v2 transport since genesis and there is no population of older nodes that
 * predates it, so a listening peer supports it. Should the assumption ever be
 * wrong for one, the connection falls back to v1 -- the cost of guessing wrong
 * is one reconnect, while the cost of not guessing is that every seeded peer is
 * first contacted unencrypted.
 */
