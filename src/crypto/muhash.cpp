...
Num3072 Num3072::GetInverse() const
{
    ...
    //   https://github.com/quarlcoin-core/secp256k1/blob/master/src/modinv32_impl.h
    // - Explanation of the algorithm:
    //   https://github.com/quarlcoin-core/secp256k1/blob/master/doc/safegcd_implementation.md
...
Num3072 MuHash3072::ToNum3072(std::span<const unsigned char> in) {
    ...
    // BLAKE3, like everything else this chain hashes. MuHash needs a random
    // oracle into the group and does not care which one; using the chain's own
    // hash means the UTXO set digest is computed with the same function as the
    // blocks it summarises -- which matters the moment that digest is committed
    // to in a header rather than only reported by an RPC.
