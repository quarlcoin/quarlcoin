...
static void SetupQuarlcoinUtilArgs(ArgsManager &argsman)
{
...
static int AppInitUtil(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " quarl-util utility version " + FormatFullVersion() + "\n";

        if (args.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The quarl-util tool provides Quarlcoin related functionality that does not rely on the ability to access a running node. Available [commands] are listed below.\n"
                "\n"
                "Usage:  quarl-util [options] [command]\n"
                "or:     quarl-util [options] grind <hex-block-header>\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
static void grind_task(uint32_t nBits, CBlockHeader header, uint32_t offset, uint32_t step, std::atomic<bool>& found, uint32_t& proposed_nonce)
{
    ...
    while (!found && header.nNonce < finish) {
        ...
        do {
            // The proof-of-work value is GetPoWHash, one SHA-256d deeper than
            // the block-identity hash GetHash.
