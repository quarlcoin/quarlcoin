        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Not `node`: that is the NodeContext above, and the argument is an address.
    if (auto node_arg{self.MaybeArg<std::string_view>("node")}) {
        bool found = false;
        for (const AddedNodeInfo& info : vInfo) {
            if (info.m_params.m_added_node == *node_arg) {
                vInfo.assign(1, info);
                found = true;
                break;
            }
    ...
    return RPCMethod{
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::STR_HEX, "file_hash", "the SHA-256d hash of the exported ASMap data"},
            }
        },
        RPCExamples{
        ...
        [&](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue {
            ...
            result.pushKV("file_hash", HexStr(hasher.GetHash()));
            return result;
#endif
        },
    };
