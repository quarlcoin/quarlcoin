                     ...
                     TxVerbosity verbosity = TxVerbosity::SHOW_DETAILS)
{
    ...
    // Call into TxToUniv() in quarlcoin-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in quarlcoin-common, so we query them here and push the
    ...
    return RPCHelpMan{
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::STR, "address", /*optional=*/true, "The Quarlcoin address (only if a well-defined address exists)"},
...
static RPCHelpMan decodepsbt()
{
    return RPCHelpMan{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed Quarlcoin transaction.",
                {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (unsigned int i = 0; i < psbtx.inputs.size(); ++i) {
        ...
        if (!input.m_proprietary.empty()) {
            // Named apart from the global-map `proprietary` above: this is the
            // input's, and while the names were shared only declaration order
            // said which one a line meant.
            UniValue input_proprietary(UniValue::VARR);
            for (const auto& entry : input.m_proprietary) {
                ...
                this_prop.pushKV("key", HexStr(entry.key));
                this_prop.pushKV("value", HexStr(entry.value));
                input_proprietary.push_back(std::move(this_prop));
            }
            in.pushKV("proprietary", std::move(input_proprietary));
        }
        ...
        if (input.unknown.size() > 0) {
            ...
            in.pushKV("unknown", std::move(input_unknowns));
        }
    ...
    for (unsigned int i = 0; i < psbtx.outputs.size(); ++i) {
        ...
        if (!output.m_proprietary.empty()) {
            UniValue output_proprietary(UniValue::VARR);
            for (const auto& entry : output.m_proprietary) {
                ...
                this_prop.pushKV("key", HexStr(entry.key));
                this_prop.pushKV("value", HexStr(entry.value));
                output_proprietary.push_back(std::move(this_prop));
            }
            out.pushKV("proprietary", std::move(output_proprietary));
        }
        ...
        if (output.unknown.size() > 0) {
            ...
            out.pushKV("unknown", std::move(output_unknowns));
        }
...
static RPCHelpMan combinepsbt()
{
    return RPCHelpMan{
        "combinepsbt",
        "Combine multiple partially signed Quarlcoin transactions into one transaction.\n"
