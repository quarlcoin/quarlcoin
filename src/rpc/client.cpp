...
//! Specify whether parameter should be parsed by quarl-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make quarl-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `quarl-cli` attempt to parse the
 ...
 * `quarl-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make quarl-cli treat them as positional parameters when `-named` is used.
 * This prevents `quarl-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so quarl-cli
 ...
 *
 * For example, the command `quarl-cli -named createwallet "my=wallet"`,
