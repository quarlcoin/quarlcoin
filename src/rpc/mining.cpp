...
static UniValue GetNetworkHashPS(int lookup, int height, const CChain& active_chain) {
    ...
    // If lookup is -1, use a day's worth of blocks.
    //
    // It used to mean "since the last difficulty change", which under ASERT is
    // the previous block and would make this answer the hashrate of one block.
    if (lookup == -1)
        lookup = std::min<int>(pb->nHeight, 24 * 60 * 60 / Params().GetConsensus().nPowTargetSpacing);
...
// NOTE: Unlike wallet RPC (which use QRL values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
