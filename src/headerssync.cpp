...
bool HeadersSyncState::ValidateAndProcessSingleHeader(const CBlockHeader& current)
{
    ...
    // adjustment maximum.
    if (!PermittedDifficultyTransition(m_consensus_params, m_current_height,
                m_last_header_received.nTime, current.nBits)) {
...
bool HeadersSyncState::ValidateAndStoreRedownloadedHeader(const CBlockHeader& header)
{
    ...
    if (!m_redownloaded_headers.empty()) {
        ...
        previous_time = m_chain_start.nTime;
    }

    if (!PermittedDifficultyTransition(m_consensus_params, next_height - 1,
                previous_time, header.nBits)) {
