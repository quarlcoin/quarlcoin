        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    // 2 witness overhead (marker and flag).
    //
    // Two and not one: with no witness discount a byte of witness overhead costs a
    // byte, where under Bitcoin's four-to-one it cost a quarter of one and a single
    // byte covered both. Estimating one here makes every selection a byte short of
    // what the signed transaction measures, which is not a rounding error -- it is
    // the difference between a wallet that can spend and a wallet that cannot.
