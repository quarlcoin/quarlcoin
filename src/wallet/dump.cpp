...
namespace wallet {
static const std::string DUMP_MAGIC = "QUARLCOIN_CORE_WALLET_DUMP";
...
// queue, which doesn't exist for the quarl-wallet. Define our own
// deleter here.
...
bool CreateFromDump(const ArgsManager& args, const std::string& name, const fs::path& wallet_path, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    if (*ver != DUMP_VERSION) {
        error = strprintf(_("Error: Dumpfile version is not supported. This version of quarl-wallet only supports version 1 dumpfiles. Got dumpfile with version %s"), version_value);
        dump_file.close();
        return false;
    }
