...
#ifndef QUARLCOIN_VALIDATION_H
#define QUARLCOIN_VALIDATION_H
...
public:
    ...
    //! The workers the overlay above fetches inputs with.
    //!
    //! Owned here because the overlay only holds a shared_ptr to it and must not
    //! outlive it, and because the pool is per-chainstate: a background
    //! chainstate validating a snapshot fetches its own inputs and should not
    //! queue behind the active one.
    std::shared_ptr<ThreadPool> m_prevoutfetch_pool GUARDED_BY(cs_main);
    ...
    void InitCache(int prevoutfetch_threads) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
};
...
public:
    ...
    /** The block this chainstate was last written to disk at, or nullptr.
     *
     *  Not separate bookkeeping: the coins database records the block it is at,
     *  and that record is only updated when the cache is flushed, so it *is*
     *  the last flush point. An index must not persist past it -- on restart the
     *  chainstate resumes here, and an index ahead of it would refer to blocks
     *  the UTXO set has not applied. */
    const CBlockIndex* GetLastFlushedBlock() EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
