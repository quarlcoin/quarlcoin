...
util::Result<int> SighashFromStr(const std::string& sighash)
{
    static const std::map<std::string, int> map_sighash_values = {
        // Quarlcoin has no SIGHASH_DEFAULT (a Taproot/BIP341 value; Quarlcoin is witness-v0 only).
    ...
    while (pc < script.end()) {
        ...
        if (0 <= opcode && opcode <= OP_PUSHDATA4) {
            if (vch.size() <= static_cast<std::vector<unsigned char>::size_type>(4)) {
                ...
                // Core decodes a DER signature here to print its sighash type.
                // Dropped when signatures were ML-DSA and not restored with them;
                // this only shortens what `decodescript` prints.
                str += HexStr(vch);
            }
        } else {
