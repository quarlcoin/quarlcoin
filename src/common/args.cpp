...
const char * const BITCOIN_CONF_FILENAME = "quarlcoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bQuarlcoin-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Quarlcoin
    //   new: C:\Users\Username\AppData\Local\Quarlcoin
    // macOS: ~/Library/Application Support/Quarlcoin
    // Unix-like: ~/.quarlcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Quarlcoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Quarlcoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Quarlcoin";
#else
    // Unix-like
    return pathRet / ".quarlcoin";
#endif
#endif
}
