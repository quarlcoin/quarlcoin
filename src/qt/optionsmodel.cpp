...
/** Call node.updateRwSetting() with Quarlcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Quarlcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/quarlcoin/quarlcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because quarlcoin 22.x will not try to read these.
...
//! Convert enabled/size values to quarlcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from quarlcoin -prune setting.
...
//! Get pruning size value to show in GUI from quarlcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayQuarlcoinUnit")) {
        settings.setValue("DisplayQuarlcoinUnit", QVariant::fromValue(QuarlcoinUnit::QRL));
    }
    QVariant unit = settings.value("DisplayQuarlcoinUnit");
    if (unit.canConvert<QuarlcoinUnit>()) {
        m_display_quarlcoin_unit = unit.value<QuarlcoinUnit>();
    } else {
        m_display_quarlcoin_unit = QuarlcoinUnit::QRL;
        settings.setValue("DisplayQuarlcoinUnit", QVariant::fromValue(m_display_quarlcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if quarlcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_quarlcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<QuarlcoinUnit>() == m_display_quarlcoin_unit) return;
    m_display_quarlcoin_unit = new_unit.value<QuarlcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayQuarlcoinUnit", QVariant::fromValue(m_display_quarlcoin_unit));
    Q_EMIT displayUnitChanged(m_display_quarlcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/quarlcoin/quarlcoin/pull/8273
    ...
    // (https://github.com/quarlcoin-core/gui/issues/567).
