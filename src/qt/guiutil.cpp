...
#include <qt/quarlcoinaddressvalidator.h>
...
static std::string DummyAddress(const CChainParams &params)
{
    ...
    case ChainType::MAIN:
        addr = "rsn1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tq2jku9f";
        break;
    case ChainType::TESTNET:
        addr = "trsn1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqa6qnlg";
        break;
    case ChainType::REGTEST:
        addr = "rsnrt1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqsr2427";
        break;
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Quarlcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new QuarlcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new QuarlcoinAddressCheckValidator(parent));
}
...
bool parseQuarlcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no quarlcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("quarlcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!QuarlcoinUnits::parse(QuarlcoinUnit::QRL, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseQuarlcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseQuarlcoinURI(uriInstance, out);
}

QString formatQuarlcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("quarlcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(QuarlcoinUnits::format(QuarlcoinUnit::QRL, info.amount, false, QuarlcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openQuarlcoinConf()
{
    ...
    /* Open quarlcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Quarlcoin.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Quarlcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Quarlcoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Quarlcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "quarlcoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("quarlcoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a quarlcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Quarlcoin\n";
        else
            optionFile << strprintf("Name=Quarlcoin (%s)\n", ChainTypeToString(chain));
