...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<QuarlcoinUnit>("QuarlcoinUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. quarlcoin_de.qm (shortcut "de" needs to be defined in quarlcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. quarlcoin_de_DE.qm (shortcut "de_DE" needs to be defined in quarlcoin.qrc)
...
static const char* qt_argv = "quarl-qt";

QuarlcoinApplication::QuarlcoinApplication()
...
void QuarlcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the QuarlcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", QuarlcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void QuarlcoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool QuarlcoinApplication::createOptionsModel(bool resetSettings)
{
...
void QuarlcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new QuarlcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &QuarlcoinGUI::quitRequested, this, &QuarlcoinApplication::requestShutdown);
...
void QuarlcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void QuarlcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool QuarlcoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void QuarlcoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &QuarlcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &QuarlcoinApplication::handleRunawayException);
    connect(this, &QuarlcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &QuarlcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void QuarlcoinApplication::parameterSetup()
{
...
void QuarlcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void QuarlcoinApplication::requestInitialize()
{
...
void QuarlcoinApplication::requestShutdown()
{
...
void QuarlcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    // quarlcoin: URIs or payment requests:
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &QuarlcoinGUI::handlePaymentRequest);
        connect(window, &QuarlcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void QuarlcoinApplication::handleRunawayException(const QString &message)
{
...
void QuarlcoinApplication::handleNonFatalException(const QString& message)
{
...
WId QuarlcoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool QuarlcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", QuarlcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(quarlcoin);
    Q_INIT_RESOURCE(quarlcoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(QUARLCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see quarl-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see quarl-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse quarlcoin.conf, determine network, switch to network specific
    ...
    // quarlcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
