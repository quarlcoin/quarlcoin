...
void PSBTOperationsDialog::openWithPSBT(PartiallySignedTransaction psbtx)
{
    ...
    if (m_wallet_model) {
        size_t n_could_sign;
        const auto err{m_wallet_model->wallet().fillPSBT(/*sighash_type=*/std::nullopt, /*sign=*/false, /*bip32derivs=*/true, &n_could_sign, *m_transaction_data, complete)};
        if (err) {
            showStatus(tr("Failed to load transaction: %1")
                           .arg(QString::fromStdString(common::PSBTErrorString(*err).translated)),
                       StatusLevel::Error);
            return;
        }
...
void PSBTOperationsDialog::signTransaction()
{
    ...
    const auto err{m_wallet_model->wallet().fillPSBT(/*sighash_type=*/std::nullopt, /*sign=*/true, /*bip32derivs=*/true, &n_signed, *m_transaction_data, complete)};

    if (err) {
        showStatus(tr("Failed to sign transaction: %1")
            .arg(QString::fromStdString(common::PSBTErrorString(*err).translated)), StatusLevel::Error);
        return;
    }
...
void PSBTOperationsDialog::saveTransaction() {
    ...
    for (const CTxOut& out : m_transaction_data->tx->vout) {
        ...
        QString amount = QuarlcoinUnits::format(m_client_model->getOptionsModel()->getDisplayUnit(), out.nValue);
...
QString PSBTOperationsDialog::renderTransaction(const PartiallySignedTransaction &psbtx)
{
    ...
    for (const CTxOut& out : psbtx.tx->vout) {
        ...
        tx_description.append(bullet_point).append(tr("Sends %1 to %2")
            .arg(QuarlcoinUnits::formatWithUnit(QuarlcoinUnit::QRL, out.nValue))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(QuarlcoinUnits::formatWithUnit(QuarlcoinUnit::QRL, *analysis.fee));

        // add total amount in all subdivision units
        tx_description.append("<hr />");
        QStringList alternativeUnits;
        for (const QuarlcoinUnits::Unit u : QuarlcoinUnits::availableUnits())
        {
        ...
        tx_description.append(QString("<b>%1</b>: <b>%2</b>").arg(tr("Total Amount"))
            .arg(QuarlcoinUnits::formatHtmlWithUnit(m_client_model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
size_t PSBTOperationsDialog::couldSignInputs(const PartiallySignedTransaction &psbtx) {
    ...
    size_t n_signed;
    bool complete;
    const auto err{m_wallet_model->wallet().fillPSBT(/*sighash_type=*/std::nullopt, /*sign=*/false, /*bip32derivs=*/false, &n_signed, *m_transaction_data, complete)};
