...
QString TransactionDesc::toHTML(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord* rec, QuarlcoinUnit unit)
{
    int numBlocks;
    interfaces::WalletTxStatus status;
    bool inMempool;
    // The order form is one list of key/value pairs, not a message list and a
    // payment-request list beside it: this chain never carried BIP70, so there
    // is no second kind of attachment for a second list to hold.
    ...
    if (wtx.is_coinbase && nCredit == 0)
    {
        ...
        if (status.is_in_main_chain)
            strHTML += QuarlcoinUnits::formatHtmlWithUnit(unit, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", status.blocks_to_maturity) + ")";
        else
    ...
    else if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + QuarlcoinUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    }
    else
    {
        ...
        if (all_from_me)
        {
            ...
            for (const CTxOut& txout : wtx.tx->vout)
            {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + QuarlcoinUnits::formatHtmlWithUnit(unit, -txout.nValue) + "<br>";
                if(toSelf)
                    strHTML += "<b>" + tr("Credit") + ":</b> " + QuarlcoinUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }
            ...
            if (all_to_me)
            {
                ...
                strHTML += "<b>" + tr("Total debit") + ":</b> " + QuarlcoinUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total credit") + ":</b> " + QuarlcoinUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }

            CAmount nTxFee = nDebit - wtx.tx->GetValueOut();
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + QuarlcoinUnits::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
        }
        else
        {
            //
            ...
            for (const CTxIn& txin : wtx.tx->vin) {
                if (*(mine++)) {
                    strHTML += "<b>" + tr("Debit") + ":</b> " + QuarlcoinUnits::formatHtmlWithUnit(unit, -wallet.getDebit(txin)) + "<br>";
                }
            }
            mine = wtx.txout_is_mine.begin();
            for (const CTxOut& txout : wtx.tx->vout) {
                if (*(mine++)) {
                    strHTML += "<b>" + tr("Credit") + ":</b> " + QuarlcoinUnits::formatHtmlWithUnit(unit, wallet.getCredit(txout)) + "<br>";
                }
            }
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + QuarlcoinUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
    ...
    // Message from normal quarlcoin:URI (quarlcoin:123...?message=example)
    for (const std::pair<std::string, std::string>& r : orderForm) {
        if (r.first == "Message") {
            strHTML += "<br><b>" + tr("Message") + ":</b><br>" + GUIUtil::HtmlEscape(r.second, true) + "<br>";
            continue;
        }
        if (r.first != "PaymentRequest") continue;
        const std::string& req{r.second};
        QString merchant;
            ...
            if (auto prev{node.getUnspentOutput(prevout)}) {
                {
                    ...
                    strHTML = strHTML + " " + tr("Amount") + "=" + QuarlcoinUnits::formatHtmlWithUnit(unit, vout.nValue);
