...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        {   // User-entered quarlcoin address / amount:
...
void WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        {
            if (!rcp.message.isEmpty()) { // Message from normal quarlcoin:URI (quarlcoin:123...?message=example)
...
bool WalletModel::bumpFee(Txid hash, Txid& new_hash)
{
    ...
    questionString.append(QuarlcoinUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("Increase:"));
    questionString.append("</td><td>");
    questionString.append(QuarlcoinUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee - old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("New fee:"));
    questionString.append("</td><td>");
    questionString.append(QuarlcoinUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee));
    ...
    if (retval == QMessageBox::Save) {
        ...
        bool complete = false;
        const auto err{wallet().fillPSBT(/*sighash_type=*/std::nullopt, /*sign=*/false, /*bip32derivs=*/true, nullptr, psbtx, complete)};
        if (err || complete) {
