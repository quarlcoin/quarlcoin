...
void CoinsViews::InitCache(int prevoutfetch_threads)
{
    AssertLockHeld(::cs_main);
    m_cacheview = std::make_unique<CCoinsViewCache>(&m_catcherview);

    // The overlay fetches a block's inputs on worker threads while the main
    // thread validates. Zero workers is a supported answer -- it means fetch on
    // the calling thread -- but the pool must still exist, because the overlay
    // asserts it has one rather than testing for it on every input.
    m_prevoutfetch_pool = std::make_shared<ThreadPool>("prevoutfetch");
    if (prevoutfetch_threads > 0) m_prevoutfetch_pool->Start(prevoutfetch_threads);
    m_connect_block_view = std::make_unique<CoinsViewOverlay>(&*m_cacheview, m_prevoutfetch_pool);
}
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (consensusParams.enforce_BIP94) {
        // Every block but genesis. Under the old rule this applied to the
        // first block of a retarget period, because that was the only timestamp
        // the difficulty read; ASERT reads the parent's timestamp on every
        // block, so the rule generalises to every block.
        if (nHeight > 0) {
...
const CBlockIndex* Chainstate::GetLastFlushedBlock()
{
    AssertLockHeld(::cs_main);
    const uint256 hash{CoinsDB().GetBestBlock()};
