 ...
 *  Quarlcoin because since the activation of BIP66, signatures are verified to be
...
static const SHA256Writer HASHER_TAPTWEAK{TaggedHash("TapTweak")};
...
uint256 XOnlyPubKey::ComputeTapTweakHash(const uint256* merkle_root) const
{
    if (merkle_root == nullptr) {
        ...
        return (SHA256Writer{HASHER_TAPTWEAK} << m_keydata).GetSHA256();
    } else {
        return (SHA256Writer{HASHER_TAPTWEAK} << m_keydata << *merkle_root).GetSHA256();
    }
}
...
bool CPubKey::Verify(const uint256 &hash, const std::vector<unsigned char>& vchSig) const {
     ...
     * not historically been enforced in Quarlcoin, so normalize them first. */
