...
bool V1Transport::SetMessageToSend(CSerializedNetMsg& msg) noexcept
{
    ...
    // create header
    //
    // Named apart from the V1Transport::hdr member it used to shadow. The two
    // are different headers -- that one is what is being received, this one is
    // what is about to be sent -- and while the name was shared, the three uses
    // below meant the local only because it was declared last.
    CMessageHeader out_hdr(m_magic_bytes, msg.m_type.c_str(), msg.data.size());
    memcpy(out_hdr.pchChecksum, hash.begin(), CMessageHeader::CHECKSUM_SIZE);

    // serialize header
    m_header_to_send.clear();
    VectorWriter{m_header_to_send, 0, out_hdr};
...
bool CConnman::BindListenPort(const CService& addrBind, bilingual_str& strError, NetPermissionFlags permissions)
{
    ...
    if (sock->Bind(reinterpret_cast<struct sockaddr*>(&sockaddr), len) == SOCKET_ERROR) {
        ...
        } else if (addrBind.GetPort() < 1024) {
            // This chain listens on 443 by default, where HTTPS lives and where
            // an ISP cannot drop the port without dropping the web. That number
            // is privileged, so a node started by an ordinary user is refused it
            // by the kernel and would otherwise exit with nothing but errno to
            // go on. Say what to do about it.
            strError = strprintf(_("Unable to bind to %s: ports below 1024 need permission the "
                                   "kernel did not grant (%s).\n"
                                   "Either allow this one binary to use them, once:\n"
                                   "    sudo setcap cap_net_bind_service=+ep %s\n"
                                   "or listen somewhere unprivileged with -port=<n>. The low port "
                                   "is camouflage, not a requirement: a node on any port is a "
                                   "full node."),
                                 addrBind.ToStringAddrPort(), NetworkErrorString(nErr), CLIENT_NAME);
        } else {
