...
int64_t GetMinimumTime(const CBlockIndex* pindexPrev)
{
    ...
    // The timewarp floor, on every block rather than on the first of a retarget
    // period. BIP 94 puts it at period boundaries because that is where the old
    // rule read a timestamp and where an attacker could therefore move one to
    // any effect. ASERT reads the parent's timestamp on every block, so every
    // block is such a boundary and the floor belongs on all of them.
