    ...
    if (state.GetResult() == TxValidationResult::TX_MISSING_INPUTS) {
        ...
        if (first_time_failure && !RecentRejectsFilter().contains(ptx->GetWitnessHash().ToUint256())) {
            ...
            if (!fRejectedParents) {
                ...
                // Not `nodeid` again: the peer this transaction came from is
                // named that in the enclosing scope, and the loop walks other
                // peers that might resolve the orphan.
                for (const auto& candidate : orphan_resolution_candidates) {
                    if (MaybeAddOrphanResolutionCandidate(unique_parents, ptx->GetWitnessHash(), candidate, now)) {
                        m_orphanage->AddTx(ptx, candidate);
                    }
                }
