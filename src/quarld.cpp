...
static bool ParseArgs(NodeContext& node, int argc, char* argv[])
{
    ...
    // config_error, not another `error`: the std::string above is still in scope
    // and holds the argument-parsing message.
    if (auto config_error = common::InitConfig(args)) {
        return InitError(config_error->message, config_error->details);
    }

    // Error out when loose non-argument tokens are encountered on command line
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see quarld -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(interfaces::Init& init, ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        if (args.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The " CLIENT_NAME " daemon (quarld) is a headless program that connects to the Quarlcoin network to validate and relay transactions and blocks, as well as relaying addresses.\n\n"
                "It provides the backbone of the Quarlcoin network and its RPC, REST and ZMQ services can provide various transaction, block and address-related services.\n\n"
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for quarld but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect quarld signal handlers
    noui_connect();
