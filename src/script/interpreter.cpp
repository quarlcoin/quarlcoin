 ...
 *
 * See https://quarlcointalk.org/index.php?topic=8392.msg127623#msg127623
 *
...
bool static IsLowDERSignature(const valtype &vchSig, ScriptError* serror) {
    ...
    // https://quarlcoin.stackexchange.com/a/12556:
...
const SHA256Writer HASHER_TAPSIGHASH{TaggedHash("TapSighash")};
const SHA256Writer HASHER_TAPLEAF{TaggedHash("TapLeaf")};
const SHA256Writer HASHER_TAPBRANCH{TaggedHash("TapBranch")};
...
bool SignatureHashSchnorr(uint256& hash_out, ScriptExecutionData& execdata, const T& tx_to, uint32_t in_pos, uint8_t hash_type, SigVersion sigversion, const PrecomputedTransactionData& cache, MissingDataBehavior mdb)
{
    ...
    SHA256Writer ss{HASHER_TAPSIGHASH};
    ...
    if (output_type == SIGHASH_SINGLE) {
        ...
        if (!execdata.m_output_hash) {
            SHA256Writer sha_single_output{};
...
{
    return (SHA256Writer{HASHER_TAPLEAF} << leaf_version << CompactSizeWriter(script.size()) << script).GetSHA256();
}

uint256 ComputeTapbranchHash(std::span<const unsigned char> a, std::span<const unsigned char> b)
{
    SHA256Writer ss_branch{HASHER_TAPBRANCH};
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, script_verify_flags flags, const BaseSignatureChecker& checker, ScriptError* serror, bool is_p2sh)
{
    ...
    if (witversion == 0) {
        ...
        if (stack.size() >= 2 && !stack.back().empty() && stack.back()[0] == ANNEX_TAG) {
            ...
            execdata.m_annex_hash = (SHA256Writer{} << annex).GetSHA256();
