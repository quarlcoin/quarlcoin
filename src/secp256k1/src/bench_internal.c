...
static void bench_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        secp256k1_hash256_write(&sha, data->data, 32);
        secp256k1_hash256_finalize(&sha, data->data);
    }
}
...
static void bench_hmac_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_hmac_hash256_initialize(&hmac, data->data, 32);
        secp256k1_hmac_hash256_write(&hmac, data->data, 32);
        secp256k1_hmac_hash256_finalize(&hmac, data->data);
    }
}
...
static void bench_rfc6979_hmac_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_rfc6979_hmac_hash256_initialize(&rng, data->data, 64);
        secp256k1_rfc6979_hmac_hash256_generate(&rng, data->data, 32);
    }
}
