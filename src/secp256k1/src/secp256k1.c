...
static int nonce_function_rfc6979(unsigned char *nonce32, const unsigned char *msg32, const unsigned char *key32, const unsigned char *algo16, void *data, unsigned int counter) {
   ...
   secp256k1_rfc6979_hmac_hash256_initialize(&rng, keydata, offset);
   for (i = 0; i <= counter; i++) {
       secp256k1_rfc6979_hmac_hash256_generate(&rng, nonce32, 32);
   }
   secp256k1_rfc6979_hmac_hash256_finalize(&rng);

   secp256k1_memclear_explicit(keydata, sizeof(keydata));
   secp256k1_rfc6979_hmac_hash256_clear(&rng);
   return 1;
}
...
int secp256k1_tagged_sha256(const secp256k1_context* ctx, unsigned char *hash32, const unsigned char *tag, size_t taglen, const unsigned char *msg, size_t msglen) {
    ...
    secp256k1_hash256_finalize(&sha, hash32);
