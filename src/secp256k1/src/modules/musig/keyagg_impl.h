...
static void secp256k1_musig_keyagglist_sha256(secp256k1_hash256 *sha) {
    static const unsigned char tag[] = "KeyAgg list";
...
static int secp256k1_musig_compute_pks_hash(const secp256k1_context *ctx, unsigned char *pks_hash, const secp256k1_pubkey * const* pks, size_t np) {
    ...
    for (i = 0; i < np; i++) {
        ...
        secp256k1_hash256_write(&sha, ser, sizeof(ser));
    }
    secp256k1_hash256_finalize(&sha, pks_hash);
    return 1;
}

/* Initializes SHA256 with fixed midstate. This midstate was computed by applying
 * SHA256 to SHA256("KeyAgg coefficient")||SHA256("KeyAgg coefficient"). */
static void secp256k1_musig_keyaggcoef_sha256(secp256k1_hash256 *sha) {
    static const unsigned char tag[] = "KeyAgg coefficient";
    secp256k1_hash256_initialize_tagged(sha, tag, sizeof(tag) - 1);
}
...
static void secp256k1_musig_keyaggcoef_internal(secp256k1_scalar *r, const unsigned char *pks_hash, secp256k1_ge *pk, const secp256k1_ge *second_pk) {
    ...
    if (!secp256k1_ge_is_infinity(second_pk)
        ...
        secp256k1_hash256_write(&sha, pks_hash, 32);
