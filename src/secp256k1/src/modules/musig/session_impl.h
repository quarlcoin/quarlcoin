...
static void secp256k1_nonce_function_musig_helper(secp256k1_hash256 *sha, unsigned int prefix_size, const unsigned char *data, unsigned char len) {
    ...
    secp256k1_hash256_write(sha, zero, prefix_size - 1);
    if (data != NULL) {
        secp256k1_hash256_write(sha, &len, 1);
        secp256k1_hash256_write(sha, data, len);
    } else {
        len = 0;
        secp256k1_hash256_write(sha, &len, 1);
    }
...
static void secp256k1_nonce_function_musig_sha256_tagged_aux(secp256k1_hash256 *sha) {
    static const unsigned char tag[] = "MuSig/aux";
    secp256k1_hash256_initialize_tagged(sha, tag, sizeof(tag) - 1);
}

/* Initializes SHA256 with fixed midstate. This midstate was computed by applying
 * SHA256 to SHA256("MuSig/nonce")||SHA256("MuSig/nonce"). */
static void secp256k1_nonce_function_musig_sha256_tagged(secp256k1_hash256 *sha) {
    static const unsigned char tag[] = "MuSig/nonce";
    secp256k1_hash256_initialize_tagged(sha, tag, sizeof(tag) - 1);
}
...
static void secp256k1_nonce_function_musig(secp256k1_scalar *k, const unsigned char *session_secrand, const unsigned char *msg32, const unsigned char *seckey32, const unsigned char *pk33, const unsigned char *agg_pk32, const unsigned char *extra_input32) {
    ...
    if (seckey32 != NULL) {
        ...
        secp256k1_hash256_write(&sha, session_secrand, 32);
        secp256k1_hash256_finalize(&sha, rand);
        for (i = 0; i < 32; i++) {
            rand[i] ^= seckey32[i];
        }
    } else {
    ...
    secp256k1_hash256_write(&sha, rand, sizeof(rand));
    ...
    secp256k1_hash256_write(&sha, &msg_present, 1);
    if (msg_present) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        secp256k1_hash256 sha_tmp = sha;
        secp256k1_hash256_write(&sha_tmp, &i, 1);
        secp256k1_hash256_finalize(&sha_tmp, buf);
        secp256k1_scalar_set_b32(&k[i], buf, NULL);

        /* Attempt to erase secret data */
        secp256k1_memclear_explicit(buf, sizeof(buf));
        secp256k1_hash256_clear(&sha_tmp);
    }
...
static void secp256k1_musig_compute_noncehash_sha256_tagged(secp256k1_hash256 *sha) {
    static const unsigned char tag[] = "MuSig/noncecoef";
    secp256k1_hash256_initialize_tagged(sha, tag, sizeof(tag) - 1);
}
...
static void secp256k1_musig_compute_noncehash(unsigned char *noncehash, secp256k1_ge *aggnonce, const unsigned char *agg_pk32, const unsigned char *msg) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        secp256k1_hash256_write(&sha, buf, sizeof(buf));
    }
    secp256k1_hash256_write(&sha, agg_pk32, 32);
    secp256k1_hash256_write(&sha, msg, 32);
    secp256k1_hash256_finalize(&sha, noncehash);
}
