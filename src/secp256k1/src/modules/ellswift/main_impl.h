...
static void secp256k1_ellswift_prng(unsigned char* out32, const secp256k1_hash256 *hasher, uint32_t cnt) {
    secp256k1_hash256 hash = *hasher;
    ...
    buf4[2] = cnt >> 16;
    buf4[3] = cnt >> 24;
    secp256k1_hash256_write(&hash, buf4, 4);
    secp256k1_hash256_finalize(&hash, out32);
...
static void secp256k1_ellswift_xelligatorswift_var(unsigned char *u32, secp256k1_fe *t, const secp256k1_fe *x, const secp256k1_hash256 *hasher) {
 ...
 * one.
 */
static void secp256k1_ellswift_elligatorswift_var(unsigned char *u32, secp256k1_fe *t, const secp256k1_ge *p, const secp256k1_hash256 *hasher) {
...
static void secp256k1_ellswift_sha256_init_encode(secp256k1_hash256* hash) {
    static const unsigned char tag[] = "secp256k1_ellswift_encode";
    secp256k1_hash256_initialize_tagged(hash, tag, sizeof(tag) - 1);
}
...
int secp256k1_ellswift_encode(const secp256k1_context *ctx, unsigned char *ell64, const secp256k1_pubkey *pubkey, const unsigned char *rnd32) {
    ...
    if (secp256k1_pubkey_load(ctx, &p, pubkey)) {
        ...
        secp256k1_hash256_write(&hash, p64, sizeof(p64));
        secp256k1_hash256_write(&hash, rnd32, 32);
...
static void secp256k1_ellswift_sha256_init_create(secp256k1_hash256* hash) {
    static const unsigned char tag[] = "secp256k1_ellswift_create";
    secp256k1_hash256_initialize_tagged(hash, tag, sizeof(tag) - 1);
}
...
int secp256k1_ellswift_create(const secp256k1_context *ctx, unsigned char *ell64, const unsigned char *seckey32, const unsigned char *auxrnd32) {
    ...
    secp256k1_hash256_write(&hash, seckey32, 32);
    secp256k1_hash256_write(&hash, zero32, sizeof(zero32));
    secp256k1_declassify(ctx, &hash, sizeof(hash)); /* private key is hashed now */
    if (auxrnd32) secp256k1_hash256_write(&hash, auxrnd32, 32);
...
static int ellswift_xdh_hash_function_prefix(unsigned char *output, const unsigned char *x32, const unsigned char *ell_a64, const unsigned char *ell_b64, void *data) {
    ...
    secp256k1_hash256_initialize(&sha);
    secp256k1_hash256_write(&sha, data, 64);
    secp256k1_hash256_write(&sha, ell_a64, 64);
    secp256k1_hash256_write(&sha, ell_b64, 64);
    secp256k1_hash256_write(&sha, x32, 32);
    secp256k1_hash256_finalize(&sha, output);
    secp256k1_hash256_clear(&sha);

    return 1;
}

/** Set hash state to the BIP340 tagged hash midstate for "bip324_ellswift_xonly_ecdh". */
static void secp256k1_ellswift_sha256_init_bip324(secp256k1_hash256* hash) {
    static const unsigned char tag[] = "bip324_ellswift_xonly_ecdh";
    secp256k1_hash256_initialize_tagged(hash, tag, sizeof(tag) - 1);
}
