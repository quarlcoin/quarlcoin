...
public:
    ...
    : m_params{chainparams.GetConsensus()}, m_caches{caches}, m_bit(bit)
    {
        if (chainparams.IsTestChain()) {
            // 2016, as on Bitcoin. It used to be read off the retarget epoch;
            // there is no epoch any more, and a signalling window never had a
            // reason to be tied to one beyond both wanting a round number.
            period = 2016;
