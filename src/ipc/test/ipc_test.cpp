...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    IpcCheck("bind invalid throws", throws_invalid_argument([&] { process->bind(datadir, "test_quarlcoin", invalid_bind); }));
    IpcCheck("connect invalid throws", throws_invalid_argument([&] { process->connect(datadir, "test_quarlcoin", invalid_bind); }));

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        int serve_fd = process->bind(datadir, "test_quarlcoin", address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        int connect_fd{process->connect(datadir, "test_quarlcoin", address)};
    ...
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath("quarlcoin_sock0_XXXXXX")),
        strprintf("unix:%s", TempPath("quarlcoin_sock1_XXXXXX")),
    };
